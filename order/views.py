...
def redirect_after_payment(request, order_id):
    ...
    try:
        ...
        redirect_url = f"krossgeorgia://payment-success/{order_id}" if order.payment_status == "completed" else "krossgeorgia://payment-failed"
...
class PaymentCallbackApiView(APIView):
    ...
    def verify_signature(self, signature, request_body):
        ...
        try:
            ...
            public_key.verify(
                decoded_signature,
                request_body,  # ⚠️ *არ იყენო `.encode()` რადგან უკვე ბინარულია!*
                padding.PKCS1v15(),
                hashes.SHA256()
            )
            return True
